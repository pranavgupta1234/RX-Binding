package pranav.apps.amazing.rxbinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    */
    ...
    private void observablesMethod1() {
        ...
        * */
        Observable<Boolean> number_change_observable = RxTextView.textChanges(number)
                .debounce(500,TimeUnit.MILLISECONDS)
         //       .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
        ...
        Observable<Boolean> username_change_observable = RxTextView.textChanges(username)
                .debounce(500,TimeUnit.MILLISECONDS)
         //       .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .map(new Func1<CharSequence, Boolean>() {
                    @Override
                    public Boolean call(CharSequence charSequence) {
                        if(charSequence.toString().contains("Pranav")){
                            return true;
                        }
                        return false;
                    }
    ...
    private void setupObservables() {
        ...
        * */
        ...
        Observable<CharSequence> phoneObservable = RxTextView.textChanges(number);
